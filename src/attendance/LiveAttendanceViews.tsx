...
function ExplanationDrawer({ detail }: { detail: AttendanceDayDetail }) {
  ...
  return <div className="timeline-detail explanation-drawer">
    ...
    <p>
      ...
      {trace.leave ? ` · ${trace.leave.leaveTypeName}${trace.leave.halfDay ? ' (half day)' : ''}` : ''}
...
export function ApprovalInboxView({ api, period, notify }: { api: ApiClient; period: ProcessingPeriod; notify: Notify }) {
  ...
  return <>
    ...
    {(loading || error || !data?.items.length) ? state : <section className="panel table-panel"><table><thead><tr><th>Request</th><th>Target</th><th>Submitted by</th><th>History</th><th>Decision</th></tr></thead><tbody>{data.items.map(request => <tr key={request.id}><td><b>{request.id.slice(0, 8)}</b><small className="subline">{label(request.type)}</small></td><td>{request.leaveRequest ? <>{request.leaveRequest.employee.firstName} {request.leaveRequest.employee.lastName}<small className="subline">{request.leaveRequest.leaveType.name} · {formatDate(request.leaveRequest.startDate)}{request.leaveRequest.startDate !== request.leaveRequest.endDate ? ` – ${formatDate(request.leaveRequest.endDate)}` : ''}{request.leaveRequest.halfDay ? ' (half day)' : ''}</small></> : request.onDutyRequest ? <>{request.onDutyRequest.employee.firstName} {request.onDutyRequest.employee.lastName}<small className="subline">{label(request.onDutyRequest.category)} · {formatDate(request.onDutyRequest.startDate)}{request.onDutyRequest.startDate !== request.onDutyRequest.endDate ? ` – ${formatDate(request.onDutyRequest.endDate)}` : ''}{request.onDutyRequest.halfDay ? ' (half day)' : ''}</small></> : request.compOffCredit ? <>{request.compOffCredit.employee.firstName} {request.compOffCredit.employee.lastName}<small className="subline">Worked {formatDate(request.compOffCredit.workedDate)} · {request.compOffCredit.creditDays} day(s) credit</small></> : request.period?.name ?? (request.exception ? (request.exception.employee ? `${request.exception.employee.firstName} ${request.exception.employee.lastName}` : String(request.exception.details?.employeeNumber ?? 'Unknown employee')) : '—')}</td><td>{request.requestedBy}</td><td>{request.actions.length} actions</td><td><div className="table-actions"><button className="approve" onClick={() => void act(request, 'APPROVED')}>Approve</button><button className="secondary small" onClick={() => void act(request, 'REJECTED')}>Reject</button></div></td></tr>)}</tbody></table></section>}
  </>
}
