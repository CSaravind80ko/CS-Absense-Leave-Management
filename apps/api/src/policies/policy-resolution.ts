import type { PolicyScopeType, PolicyVersion } from '@prisma/client';
import type { PrismaService } from '../prisma/prisma.service';

/**
 * NOTE: this precedence-resolution algorithm is intentionally duplicated in
 * apps/worker/src/policy-resolution.ts (against a Prisma.TransactionClient rather than
 * PrismaService). There is no shared source package between apps/api and apps/worker
 * today beyond @attendance/contracts (event schemas only), so this ~30-line algorithm is
 * implemented once per app rather than introducing a new shared package for it. Mirrored
 * unit test suites in both apps/api/test/policies.service.spec.ts and
 * apps/worker/test/policy-resolution.spec.ts guard against the two copies drifting apart.
 */
